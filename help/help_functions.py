...
def nip_generate(how_much):
    ...
    nip = {}
    we = (6, 5, 7, 2, 3, 4, 5, 6, 7)
    for el in range(how_much):
        elem, office = choice(list(us.items()))
        x = [randint(0, 9) for p in range(0, 6)]
        for i in str(elem)[::-1]:
            x.insert(0, int(i))
        if sum([a * b for a, b in zip(we, x)]) % 11 != 10:
            x.append(sum([a * b for a, b in zip(we, x)]) % 11)
            x = ''.join(map(str, x))
            nip[x] = office
        else:
            if x[8] in range(0, 8):
